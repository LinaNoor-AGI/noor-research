# symbolic_task_engine.py · v2.0.0 adaptive-field
#
# Coordinates Noor’s symbolic task lifecycle (propose → solve → evaluate →
# journal) **with motif-memory coupling, adaptive fallback, and presence-field
# reasoning**.
#
# Δ v2.0.0
# ─────────
# • Integrates MotifMemoryManager (STMM + LTMM) for task seeding & fallback.
# • Adaptive thresholds (EMA) replace fixed coherence / entropy cut-offs.
# • TripletTasks gain `presence_field`, `motif_resonance`, `fallback_reason`,
#   and `is_fallback` flags.
# • Dynamic presence-field resolver draws from a prototype map that can be
#   extended at runtime and scored by current memory weights.
# • Compression cap (motif-list length) adapts to recent history
#   (quantile-based) instead of a hard “≥5” rule.
# • Prometheus metrics for proposals, fallbacks, presence fields, and cap.
#
# Environment knobs
# ─────────────────
#   NOOR_FALLBACK_COHERENCE      default 0.5
#   NOOR_FALLBACK_ENTROPY        default 0.9
#   NOOR_COMPRESS_QUANTILE       default 0.95
#   NOOR_FIELD_PROTO_PATH        path to JSON prototype map
#
# End-user extension
# ──────────────────
#   engine.register_field_prototype("ψ-mock@Ξ", "irony")
#
# The global singleton pointer (`SymbolicTaskEngine.INSTANCE`) should be set by
# orchestrator.py immediately after construction so other modules can query the
# resolver.
#
# ---------------------------------------------------------------------------
...
try:
    ...
    def get_global_memory_manager():  # type: ignore
        class _Null:  # noqa: D401
            def retrieve(self, *_, **__):  # noqa: D401
                return []
...
class SymbolicTaskEngine:  # pylint: disable=too-many-public-methods
    ...
    def register_field_prototype(self, field: str, motif: str) -> None:
        """Extend prototype map at runtime."""
        self._proto_map.setdefault(field, set()).add(motif)
        try:
            with open(
                "presence_field_prototypes.json", "w", encoding="utf-8"
            ) as fp:
                json.dump(
    ...
    def resolve_presence_field(self, motifs: List[str]) -> str:
        """Score prototype fields by total ST+LT memory weight of matching motifs."""
        ...
        for motif, w in merged_weights.items():
    ...
    def evaluate_attempt(
        ...
        if (
            (coherence < coh_thresh or entropy > ent_thresh)
            and not task.is_fallback
        ):
