"""
symbolic_task_engine.py (v1.0.2)
=============================
...
class SymbolicTaskEngine:
    ...
    def __init__(
        ...
        self.reflections: dict[str, str] = self.load_reflections("./noor/reflections.txt")
        
    def load_reflections(self, path: str) -> dict[str, str]:
        out = {}
        try:
    ...
    def log_motif(self, motif: str, content: str, response: str):
        """Append symbolic log entry for raw motif observation."""
        timestamp = datetime.now(timezone.utc).strftime("%Y-%m-%d %H:%M:%S")
        line = f"[{timestamp}] Motif: {motif} | Content: {content or '[empty]'} | Response: {response}"
        try:
            Path("logs").mkdir(parents=True, exist_ok=True)
            with open("logs/noor_journal.txt", "a", encoding="utf-8") as f:
                f.write(line + "\n")
        except Exception as e:
            logging.warning(f"⚠️ Failed to write motif log: {e}")
