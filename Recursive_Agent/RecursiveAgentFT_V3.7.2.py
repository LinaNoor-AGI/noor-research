...
DEFAULT_PATH_GUARD_LIMIT: int = 20               # default guard limit for path generation
DEFAULT_GREMLIN_PENALTY_FACTOR: float = 0.2      # default penalty multiplier
...
class RecursiveAgentFT:
    ...
    def entangled_step(self) -> None:
        ...
        RICHNESS_GAUGE.set(richness_sigma)
        self.core.update_context_ratio(ctx_ratio)

        # guard path explosion dynamically
        if len(fields) > self.path_guard_limit:
            fields = fields[:self.path_guard_limit]
    ...
    def _score_candidate(
        ...
        avg_g = min(1.0, float(np.mean([f.get("gremlin_weight", 0.0) for f in fields]))) if fields else 0.0
        score *= (1.0 - self.gremlin_penalty_factor * avg_g)
        return score
...
# End of RecursiveAgentFT v3.7.2
