...
Δ v3.3.1
────────
• Warns once at startup if no HMAC secret is configured.  
• `_memory()` helper centralises lazy import handling.  
• `BOOST_BASE` (env-tunable) and `MEM_CAP_WARN` soft cap.  
• Adaptive boost now uses the class-level `BOOST_BASE`.  
• Skips boosts (but still counts them) when STMM + LTMM exceed soft cap.  
• Expanded docstrings for `get_latest_tick` / `get_latest_change`.
"""
