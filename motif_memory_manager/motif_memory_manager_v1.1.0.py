# motif_memory_manager.py · v1.1.0
#
...
# simple Jaccard similarity for ranking competing clusters
...
class MotifMemoryManager:
    ...
    def __init__(
        ...
        *,
        ...
        reef_path: str | Path | None = "/mnt/data/index.REEF",
    ...
    def complete_dyad(self, dyad: Tuple[str, str], *, top_k: int = 1) -> List[str]:
        ...
        in any REEF reflection cluster, ranked by Jaccard then LTMM weight.
        """
        ...
        for cl in clusters.values():
            if {m1, m2}.issubset(cl):
                for cand in cl:
                    if cand not in key:
                        rank = _jaccard(cl, {m1, m2}) + ltmm.get(cand, 0.0)
            ...
            with self._reef_path.open("r", encoding="utf-8") as fp:
                for line in fp:
                    ...
                    if s.startswith("motif_id"):
                        # use hash of line as cluster key
                        key = hashlib.sha1(s.encode()).hexdigest()[:12]
                        motifs = s.split("=", 1)[1].strip().lower().split()
                        clusters[key] = set(motifs)
                    elif s.startswith("expression"):
                        continue
        except Exception as exc:  # pragma: no cover
            self._log("reef_parse_error", "n/a", 0, exc=str(exc))
