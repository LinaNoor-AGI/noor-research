...
• Prometheus metrics (dyad ratio, step latency, gate usage, Möbius ruptures)
...
except ModuleNotFoundError:                 # fallback for dev / tests
    class _Faux:
        gate_overlay = None
        max_curvature = 1.0
        zeno_decay = 0.9
...
def _apply_verse_bias(state: np.ndarray, verse: str, time_step: int, ctx_ratio: float) -> np.ndarray:
    if not verse:
        return state
    bias_scale = _SETTINGS.verse_bias_strength * math.exp(-0.0001 * time_step) * (ctx_ratio ** 2)
