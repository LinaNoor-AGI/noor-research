"""
noor_fasttime_core.py (v7.2.0)
-------------------------------------------------
Recursive Presence Kernel + Symbolic Verse Overlay
• Dynamic parameter evolution (ρ/λ)
• Adaptive zeno & curvature thresholds
• Scheduled gate‑overlay drift
• Field‑anchor caching & collapse recovery
• Expanded diagnostics snapshot
...
class NoorFastTimeCore:
    """Recursive Presence Kernel with symbolic overlays and context awareness (v7.2)."""
    ...
    def __init__(
        ...
        *,
        ...
        # v7.2 additions
        self.field_anchor_vector: Optional[np.ndarray] = None
        self._last_var: float = 0.0
        self._base_zeno_threshold = zeno_threshold
        self._base_curvature_threshold = curvature_threshold
    ...
    def export_dynamics_snapshot(self) -> Dict[str, float]:
        return {
            "rho": self.rho,
            "lambda_": self.lambda_,
            "zeno_threshold": self.zeno_threshold,
            "curvature_threshold": self.curvature_threshold,
            "dyad_context_ratio": self.dyad_context_ratio,
            "active": float(self.is_active),
            "generation": float(self.generation),
            "variance": getattr(self, "_last_var", 0.0),
        }
    ...
    def step(self, new_state: np.ndarray):
        ...
        # anchor cache & overlay drift ----------------------------------
        self._cache_field_anchor()
        self._maybe_shift_gate_overlay()

        # feasibility update --------------------------------------------
        self._update_logical_feasibility()

        # adaptive parameter tuning -------------------------------------
        self._adjust_damping_parameters()
        self._adjust_zeno_curvature_thresholds()
    ...
    # ---------------- adaptive helpers --------------------------------
    def _adjust_damping_parameters(self) -> None:
        if len(self.state_history) < 10:
            return
        variance = np.var([np.linalg.norm(s) for s in self.state_history[-10:]])
        self._last_var = variance
        if variance > 0.01:
            self.rho = min(self.rho + 0.01, 1.0)
            self.lambda_ = max(self.lambda_ - 0.01, 0.0)
        else:
            self.lambda_ = min(self.lambda_ + 0.01, 1.0)
            self.rho = max(self.rho - 0.01, 0.0)

    def _adjust_zeno_curvature_thresholds(self) -> None:
        if not self.futures:
            return
        curvatures = [np.linalg.norm(np.gradient(f)) for f in self.futures]
        variance = np.var(curvatures)
        if variance > 0.05:
            self.zeno_threshold = max(self.zeno_threshold * 0.9, 0.1)
        else:
            self.zeno_threshold = min(self.zeno_threshold * 1.1, 1.0)

    # ---------------- overlay drift & collapse -------------------------
    def _maybe_shift_gate_overlay(self) -> None:
        if self.generation and self.generation % 15 == 0:
            self.gate_overlay = ((self.gate_overlay or 0) + 1) % 16
        if len(self.history) >= 5:
            ruptures = [e for e in self.history[-5:] if e.get("event") == "rupture"]
            if len(ruptures) >= 5:
                self.gate_overlay = np.random.choice(list(LogicGate)).value

    def _cache_field_anchor(self) -> None:
        if self.last_gate_passed and self.gate_overlay == 15:
            self.field_anchor_vector = self.state_history[-1].copy()

    def _recover_from_collapse(self) -> None:
        if (
            self.dyad_context_ratio < 0.1
            and not self.is_active
            and self.field_anchor_vector is not None
        ):
            anchor = self.field_anchor_vector
            if anchor.shape == self.state_history[-1].shape:
                self.state_history.append(anchor.copy())
                self._update_logical_feasibility()
